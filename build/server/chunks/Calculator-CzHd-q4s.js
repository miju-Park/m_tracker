import { d as db } from './firebase-BKeGrI0b.js';
import 'firebase/auth';
import { d as derived, r as readable, w as writable, o as onMount, b as readonly } from './index-C6mxKevJ.js';
import dayjs from 'dayjs';
import { collection, query, orderBy, getDocs, doc, setDoc } from 'firebase/firestore';
import { c as create_ssr_component, f as compute_rest_props, v as validate_component, i as spread, k as escape_attribute_value, j as escape_object, b as add_attribute, a as subscribe, h as createEventDispatcher, d as each, e as escape, s as setContext, g as getContext, l as get_store_value, p as set_current_component, r as run_all, o as onDestroy, q as current_component } from './ssr-8qQtB6gu.js';
import { clsx } from 'clsx';
import { isSameMonth, isSameDay, isToday, getLocalTimeZone, CalendarDateTime, CalendarDate, ZonedDateTime, parseZonedDateTime, parseDateTime, parseDate, toCalendar, startOfMonth, endOfMonth, getDayOfWeek as getDayOfWeek$1, DateFormatter } from '@internationalized/date';
...
const transactionHandlers = {
  getAll: async (userId, year, month) => {
...
const groupedTransations = derived(transactionStore, ($transactionStore) => {
...
const incomeCategories = [
  { category: "고정 수입", icon: "💼", color: "#6A7FDB" },
  { category: "공모주 수입", icon: "📈", color: "#57E2E5" },
  { category: "앱테크 수입", icon: "📲", color: "#45CB85" },
  { category: "기타 수입", icon: "📪", color: "#153131" }
];
const expenseCategories = [
  { category: "식비", subCategory: "마트, 장보기", icon: "🍔", color: "#E08DAC" },
  { category: "식비", subCategory: "배달, 외식", icon: "🍔", color: "#E08DAC" },
  { category: "식비", subCategory: "카페", icon: "🍔", color: "#E08DAC" },
  { category: "식비", subCategory: "술, 유흥", icon: "🍔", color: "#E08DAC" },
  { category: "패션/미용", subCategory: "의류", icon: "👚", color: "#F06C9B" },
  { category: "패션/미용", subCategory: "헤어/뷰티", icon: "👚", color: "#F06C9B" },
  { category: "패션/미용", subCategory: "세탁비", icon: "👚", color: "#F06C9B" },
  { category: "교통", subCategory: "대중교통", icon: "🚆", color: "#F9C22E" },
  { category: "교통", subCategory: "택시", icon: "🚆", color: "#F9C22E" },
  { category: "교통", subCategory: "주유", icon: "🚆", color: "#F9C22E" },
  { category: "교통", subCategory: "자동차관련", icon: "🚆", color: "#F9C22E" },
  { category: "교통", subCategory: "기차", icon: "🚆", color: "#F9C22E" },
  { category: "생필품", subCategory: "생필품", icon: "🛒", color: "#96C9DC" },
  { category: "생필품", subCategory: "자동차용품", icon: "🛒", color: "#96C9DC" },
  { category: "문화생활", subCategory: "여행", icon: "🕹️", color: "#61A0AF" },
  { category: "문화생활", subCategory: "문화생활", icon: "🕹️", color: "#61A0AF" },
  { category: "주거/통신", subCategory: "대출이자", icon: "🏠", color: "#F9B9B7" },
  { category: "주거/통신", subCategory: "관리비", icon: "🏠", color: "#F9B9B7" },
  { category: "주거/통신", subCategory: "가스비", icon: "🏠", color: "#F9B9B7" },
  { category: "주거/통신", subCategory: "핸드폰", icon: "🏠", color: "#F9B9B7" },
  { category: "주거/통신", subCategory: "인터넷", icon: "🏠", color: "#F9B9B7" },
  { category: "주거/통신", subCategory: "보험", icon: "🏠", color: "#F9B9B7" },
  { category: "부모님", subCategory: "부모님", icon: "👵🏻", color: "#F5D491" },
  { category: "경조사", subCategory: "경조사", icon: "🎁", color: "#B0F2B4" },
  { category: "경조사", subCategory: "모임회비", icon: "🎁", color: "#B0F2B4" },
  { category: "경조사", subCategory: "선물", icon: "🎁", color: "#B0F2B4" },
  { category: "건강", subCategory: "병원", icon: "🧘🏻‍♀️", color: "#BAF2E9" },
  { category: "건강", subCategory: "약국", icon: "🧘🏻‍♀️", color: "#BAF2E9" },
  { category: "건강", subCategory: "건강식품", icon: "🧘🏻‍♀️", color: "#BAF2E9" },
  { category: "앱테크지출", subCategory: "-", icon: "🐻", color: "#BAD7F2" }
];
function cn(...inputs) {
...
const parseCategoryInfo = (inputStr) => {
  ...
  return {
    category,
    icon: [...incomeCategories, ...expenseCategories].find((v) => v.category === category)?.icon ?? "",
    color: [...incomeCategories, ...expenseCategories].find((v) => v.category === category)?.color ?? ""
  };
};
...
function removeScroll(_document) {
  ...
  const { documentElement, body } = doc2;
...
function getLastFirstDayOfWeek(date, firstDayOfWeek, locale) {
  const day = getDayOfWeek$1(date, locale);
...
const css = {
  code: ".calculator.svelte-xhtb2c{display:flex;flex-direction:column;align-items:center;width:300px;margin:50px auto}.display.svelte-xhtb2c{width:100%;background-color:#333;color:white;font-size:2em;text-align:right;padding:10px;margin-bottom:10px}.buttons.svelte-xhtb2c{display:grid;grid-template-columns:repeat(4, 75px);gap:5px}button.svelte-xhtb2c{padding:10px;font-size:18px;border:none;cursor:pointer;border-radius:5px;font-family:Arial, sans-serif;text-align:center}",
  map: `{"version":3,"file":"Calculator.svelte","sources":["Calculator.svelte"],"sourcesContent":["<script lang=\\"ts\\">import { createEventDispatcher } from \\"svelte\\";\\nimport { writable } from \\"svelte/store\\";\\nlet expression = writable(\\"\\");\\nlet result = writable(\\"\\");\\nconst dispatch = createEventDispatcher();\\nconst updateExpression = (value) => {\\n  expression.update((prev) => prev + value);\\n  calculateResult();\\n};\\nconst removeLastExpression = () => {\\n  expression.update((prev) => prev?.length >= 1 ? prev.slice(0, -1) : \\"\\");\\n  calculateResult();\\n};\\nconst clearAll = () => {\\n  expression.set(\\"\\");\\n  result.set(\\"\\");\\n};\\nconst calculateResult = () => {\\n  expression.subscribe((exp) => {\\n    try {\\n      result.set(eval(exp) || \\"\\");\\n    } catch (e) {\\n      result.set(\\"\\");\\n    }\\n  });\\n};\\nconst handleKeyDown = (e) => {\\n  const allowedKeys = \\"0123456789+-*/()\\";\\n  const keyCode = e.key;\\n  if (allowedKeys.includes(keyCode)) {\\n    updateExpression(keyCode);\\n  } else if (keyCode === \\"Backspace\\") {\\n    removeLastExpression();\\n  } else if (keyCode === \\"Enter\\") {\\n    calculateResult();\\n  }\\n};\\n<\/script>\\n\\n<svelte:document on:keydown={handleKeyDown} />\\n\\n<div class=\\"calculator\\">\\n\\t<div class=\\"display\\">\\n\\t\\t<div>\\n\\t\\t\\t<!-- 수식 -->\\n\\t\\t\\t{#each $expression as char}\\n\\t\\t\\t\\t{char}\\n\\t\\t\\t{/each}\\n\\t\\t</div>\\n\\t\\t<div>\\n\\t\\t\\t<!-- 결과 -->\\n\\t\\t\\t= {$result ? Number($result).toLocaleString() : $result}\\n\\t\\t</div>\\n\\t</div>\\n\\t<div class=\\"buttons\\">\\n\\t\\t<button on:click|stopPropagation={clearAll}>AC</button>\\n\\t\\t<div />\\n\\t\\t<div />\\n\\t\\t<button on:click|stopPropagation={removeLastExpression}>🔙</button>\\n\\t\\t{#each ['7', '8', '9', '+', '4', '5', '6', '-', '1', '2', '3', '*', '0', '(', ')', '/'] as value}\\n\\t\\t\\t<button class=\\"\\" on:click|stopPropagation={() => updateExpression(value)}>{value}</button>\\n\\t\\t{/each}\\n\\t</div>\\n\\t<button\\n\\t\\ton:click|stopPropagation={() => {\\n\\t\\t\\tdispatch('submit', { amount: Number($result) });\\n\\t\\t}}>OK</button\\n\\t>\\n</div>\\n\\n<style>\\n\\t.calculator {\\n\\t\\tdisplay: flex;\\n\\t\\tflex-direction: column;\\n\\t\\talign-items: center;\\n\\t\\twidth: 300px;\\n\\t\\tmargin: 50px auto;\\n\\t}\\n\\t.display {\\n\\t\\twidth: 100%;\\n\\t\\tbackground-color: #333;\\n\\t\\tcolor: white;\\n\\t\\tfont-size: 2em;\\n\\t\\ttext-align: right;\\n\\t\\tpadding: 10px;\\n\\t\\tmargin-bottom: 10px;\\n\\t}\\n\\t.buttons {\\n\\t\\tdisplay: grid;\\n\\t\\tgrid-template-columns: repeat(4, 75px);\\n\\t\\tgap: 5px;\\n\\t}\\n\\tbutton {\\n\\t\\tpadding: 10px;\\n\\t\\tfont-size: 18px;\\n\\t\\tborder: none;\\n\\t\\tcursor: pointer;\\n\\t\\tborder-radius: 5px;\\n\\t\\tfont-family: Arial, sans-serif;\\n\\t\\ttext-align: center;\\n\\t}\\n</style>\\n"],"names":[],"mappings":"AAuEC,yBAAY,CACX,OAAO,CAAE,IAAI,CACb,cAAc,CAAE,MAAM,CACtB,WAAW,CAAE,MAAM,CACnB,KAAK,CAAE,KAAK,CACZ,MAAM,CAAE,IAAI,CAAC,IACd,CACA,sBAAS,CACR,KAAK,CAAE,IAAI,CACX,gBAAgB,CAAE,IAAI,CACtB,KAAK,CAAE,KAAK,CACZ,SAAS,CAAE,GAAG,CACd,UAAU,CAAE,KAAK,CACjB,OAAO,CAAE,IAAI,CACb,aAAa,CAAE,IAChB,CACA,sBAAS,CACR,OAAO,CAAE,IAAI,CACb,qBAAqB,CAAE,OAAO,CAAC,CAAC,CAAC,IAAI,CAAC,CACtC,GAAG,CAAE,GACN,CACA,oBAAO,CACN,OAAO,CAAE,IAAI,CACb,SAAS,CAAE,IAAI,CACf,MAAM,CAAE,IAAI,CACZ,MAAM,CAAE,OAAO,CACf,aAAa,CAAE,GAAG,CAClB,WAAW,CAAE,KAAK,CAAC,CAAC,UAAU,CAC9B,UAAU,CAAE,MACb"}`
};
const Calculator = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let $expression, $$unsubscribe_expression;
  let $result, $$unsubscribe_result;
  let expression = writable("");
  $$unsubscribe_expression = subscribe(expression, (value) => $expression = value);
  let result = writable("");
  $$unsubscribe_result = subscribe(result, (value) => $result = value);
  createEventDispatcher();
  $$result.css.add(css);
  $$unsubscribe_expression();
  $$unsubscribe_result();
  return ` <div class="calculator svelte-xhtb2c"><div class="display svelte-xhtb2c"><div> ${each($expression, (char) => {
    return `${escape(char)}`;
  })}</div> <div>
			= ${escape($result ? Number($result).toLocaleString() : $result)}</div></div> <div class="buttons svelte-xhtb2c"><button class="svelte-xhtb2c" data-svelte-h="svelte-ni1rle">AC</button> <div></div> <div></div> <button class="svelte-xhtb2c" data-svelte-h="svelte-1n7y78i">🔙</button> ${each(["7", "8", "9", "+", "4", "5", "6", "-", "1", "2", "3", "*", "0", "(", ")", "/"], (value) => {
    return `<button class=" svelte-xhtb2c">${escape(value)}</button>`;
  })}</div> <button class="svelte-xhtb2c" data-svelte-h="svelte-1fe84zh">OK</button> </div>`;
});

export { isElement as $, dequal as A, isObject as B, Calculator as C, stripValues as D, makeElement as E, disabledAttr as F, executeCallbacks as G, addMeltEventListener as H, Input as I, kbd as J, isHTMLButtonElement as K, Label as L, isHTMLElement as M, last as N, FIRST_LAST_KEYS as O, Popover_content as P, isElementDisabled as Q, Root as R, back as S, Trigger as T, forward as U, prev as V, next as W, useEscapeKeydown as X, styleToString as Y, effect as Z, usePopper as _, authStore as a, getPortalDestination as a0, createLabel as a1, generateId as a2, createHiddenInput as a3, safeOnMount as a4, isBrowser as a5, removeScroll as a6, getElementByMeltId as a7, isHTMLLabelElement as a8, isHTMLInputElement as a9, noop as aa, wrapArray as ab, toggle as ac, sleep as ad, transactionStore as b, getDayOfWeek as c, cn as d, expenseCategories as e, createDispatcher as f, groupedTransations as g, flyAndScale as h, incomeCategories as i, createBitAttrs as j, getOptionUpdater as k, tick as l, cubicOut as m, createCalendar as n, getPositioningUpdater as o, parseCategoryInfo as p, nanoid as q, removeUndefined as r, overridable as s, transactionHandlers as t, toWritableStores as u, omit as v, withGet as w, createElHelpers as x, generateIds as y, derivedVisible as z };
//# sourceMappingURL=Calculator-CzHd-q4s.js.map
